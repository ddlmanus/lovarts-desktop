...
import { FPS_PRESETS } from "@/opencut-classic/fps/presets";
import {
  floatToFrameRate,
  frameRateToFloat,
} from "@/opencut-classic/fps/utils";
...
import {
  ...
  TabsList,
  TabsTrigger,
} from "@/opencut-classic/components/ui/tabs";
...
import { useEditorStore } from "@/opencut-classic/editor/editor-store";
import { usePropertyDraft } from "@/opencut-classic/components/editor/panels/properties/hooks/use-property-draft";
import { HugeiconsIcon } from "@hugeicons/react";
import { Tick02Icon } from "@hugeicons/core-free-icons";
import { cn } from "@/opencut-classic/utils/ui";
import { dimensionToAspectRatio } from "@/opencut-classic/utils/geometry";
import { formatNumberForDisplay } from "@/opencut-classic/utils/math";
import { OcSquarePlusIcon } from "@/opencut-classic/components/icons";
...
export function SettingsView() {
  ...
  const displayProjectName =
    activeProject.metadata.name === "Lovarts Project"
      ? t("freeTools.mediaTrimmer.editor.defaultProjectName")
        ...
        >
          <TabsList>
            <TabsTrigger value="project-info">
              {t("freeTools.mediaTrimmer.editor.projectInfo")}
            </TabsTrigger>
            <TabsTrigger value="background">
              {t("freeTools.mediaTrimmer.editor.background")}
            </TabsTrigger>
          </TabsList>
        </Tabs>
      }
      ...
      {view === "project-info" && (
        <div className="flex flex-col">
          <Section showTopBorder={false}>
            <SectionHeader>
              <SectionTitle className="flex-1">
                {t("freeTools.mediaTrimmer.editor.name")}
              </SectionTitle>
              <span className="text-sm truncate">{displayProjectName}</span>
            </SectionHeader>
          </Section>
          <Section showTopBorder={false}>
            <SectionHeader className="justify-between">
              <SectionTitle className="flex-1">
                {t("freeTools.mediaTrimmer.editor.frameRate")}
              </SectionTitle>
              <Select
                value={String(
              ...
              >
                <SelectTrigger className="bg-transparent border-none p-1 h-auto">
                  <SelectValue
                    placeholder={t(
                      "freeTools.mediaTrimmer.editor.selectFrameRate",
                    )}
                  />
                </SelectTrigger>
                <SelectContent>
          ...
          >
            <SectionHeader>
              <SectionTitle className="flex-1">
                {t("freeTools.mediaTrimmer.editor.aspectRatio")}
              </SectionTitle>
            </SectionHeader>
            <SectionContent className="px-2 flex flex-col gap-1 pb-2">
              ...
              <div className="pb-2">
                <AspectRatioItem
                  key="custom"
                  label={t("freeTools.mediaTrimmer.editor.custom")}
                  ...
                  uiOptions={
                    <div className=" flex items-center gap-2 text-foreground">
                      <NumberField
                        ...
                        className="w-full"
                        aria-label={t(
                          "freeTools.mediaTrimmer.editor.canvasWidth",
                        )}
                      ...
                      <NumberField
                        ...
                        className="w-full"
                        aria-label={t(
                          "freeTools.mediaTrimmer.editor.canvasHeight",
                        )}
