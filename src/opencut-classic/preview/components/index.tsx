...
import { useRafLoop } from "@/opencut-classic/hooks/use-raf-loop";
import { useContainerSize } from "@/opencut-classic/hooks/use-container-size";
import { useFullscreen } from "@/opencut-classic/hooks/use-fullscreen";
import { CanvasRenderer } from "@/opencut-classic/services/renderer/canvas-renderer";
import { TICKS_PER_SECOND } from "@/opencut-classic/wasm";
import type { RootNode } from "@/opencut-classic/services/renderer/nodes/root-node";
import { buildScene } from "@/opencut-classic/services/renderer/scene-builder";
...
export function PreviewPanel({
  ...
  return (
    <div
      ...
      className="panel bg-background relative flex size-full min-h-0 min-w-0 flex-col rounded-none border"
    >
      <PreviewCanvas
        container={container}
...
function RenderTreeController() {
  ...
  const tracks = useEditor(
    (e) => e.timeline.getPreviewTracks() ?? e.scenes.getActiveSceneOrNull()?.tracks,
  );
  ...
  useDeepCompareEffect(() => {
    if (!activeProject || !activeScene || !tracks || width === undefined || height === undefined) return;
  ...
  }, [tracks, mediaAssets, activeProject?.settings.background, activeScene?.id, width, height]);
