...
import { OcShapesIcon, OcVideoIcon } from "@/opencut-classic/components/icons";
import {
  ContextMenu,
  ContextMenuContent,
  ContextMenuItem,
...
import { useTimelineZoom } from "@/opencut-classic/timeline/hooks/use-timeline-zoom";
import {
  useCallback,
  useEffect,
  useMemo,
...
import { useContainerSize } from "@/opencut-classic/hooks/use-container-size";
import type { MediaTime } from "@/opencut-classic/wasm";
import type { ElementDragView, DropTarget } from "@/opencut-classic/timeline";
import { TimelineTrackContent } from "./timeline-track";
import { TimelinePlayhead } from "./timeline-playhead";
import { SelectionBox } from "@/opencut-classic/selection/selection-box";
import { useBoxSelect } from "@/opencut-classic/selection/hooks/use-box-select";
import { SnapIndicator } from "./snap-indicator";
import type { SnapPoint } from "@/opencut-classic/timeline/snapping";
import type { TimelineTrack } from "@/opencut-classic/timeline";
import {
...
import { useElementInteraction } from "@/opencut-classic/timeline/hooks/element/use-element-interaction";
import {
  canTrackHaveAudio,
  canTrackBeHidden,
  getTimelineZoomMin,
...
import { timelineTimeToPixels } from "@/opencut-classic/timeline/pixel-utils";
import {
  getTrackHeight,
...
import { useTimelineSeek } from "@/opencut-classic/timeline/hooks/use-timeline-seek";
import { useTimelineDragDrop } from "@/opencut-classic/timeline/hooks/use-timeline-drag-drop";
import { TimelineRuler } from "./timeline-ruler";
import {
  TimelineBookmarksRow,
  useBookmarkDrag,
} from "@/opencut-classic/timeline/bookmarks/index";
import { useEdgeAutoScroll } from "@/opencut-classic/timeline/hooks/use-edge-auto-scroll";
import { useInitialScrollBottom } from "@/opencut-classic/timeline/hooks/use-initial-scroll-bottom";
import { useTimelineResize } from "@/opencut-classic/timeline/hooks/use-timeline-resize";
...
import { useScrollPosition } from "@/opencut-classic/timeline/hooks/use-scroll-position";
import { useTimelinePlayhead } from "@/opencut-classic/timeline/hooks/use-timeline-playhead";
...
export function Timeline() {
  ...
  return (
    <section
      ...
      {...dragProps}
      aria-label={t("freeTools.mediaTrimmer.editor.timeline")}
    >
      <TimelineToolbar
        zoomLevel={zoomLevel}
        minZoom={minZoomLevel}
      ...
      {sortedTracks.map(({ track, index }) => (
        <ContextMenu key={track.id}>
          ...
          <ContextMenuContent className="w-40">
            ...
            >
              {t("freeTools.mediaTrimmer.editor.contextMenu.pasteElements")}
            </ContextMenuItem>
            <ContextMenuItem
            ...
            >
              {canTrackHaveAudio(track) && track.muted
                ? t("freeTools.mediaTrimmer.editor.contextMenu.unmuteTrack")
                : t("freeTools.mediaTrimmer.editor.contextMenu.muteTrack")}
            </ContextMenuItem>
            <ContextMenuItem
            ...
            >
              {canTrackBeHidden(track) && track.hidden
                ? t("freeTools.mediaTrimmer.editor.contextMenu.showTrack")
                : t("freeTools.mediaTrimmer.editor.contextMenu.hideTrack")}
            </ContextMenuItem>
            {track.id !== mainTrackId && (
              <ContextMenuItem
                ...
                variant="destructive"
              >
                {t("freeTools.mediaTrimmer.editor.contextMenu.deleteTrack")}
              </ContextMenuItem>
            )}
          </ContextMenuContent>
        </ContextMenu>
