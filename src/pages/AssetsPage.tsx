...
export function AssetsPage() {
  ...
  const handleCustomize = useCallback(
    async (asset: AssetMetadata) => {
      ...
      const targetRoute = getWorkspaceRoute(workspace, asset.modelId);
      ...
      if (asset.predictionId) {
        ...
        if (localEntry?.inputs && Object.keys(localEntry.inputs).length > 0) {
          createTab(model, localEntry.inputs, initialOutputs, predictionResult, workspace);
        ...
        if (historyFormValues) {
          createTab(model, historyFormValues, initialOutputs, predictionResult, workspace);
          setPreviewAsset(null);
          navigate(targetRoute);
          return;
        }
      ...
      if (asset.predictionId) {
        ...
        try {
          ...
          createTab(model, undefined, initialOutputs, predictionResult, workspace);
          setPreviewAsset(null);
          navigate(targetRoute);
        } finally {
