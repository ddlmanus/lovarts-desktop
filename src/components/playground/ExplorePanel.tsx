...
interface ExplorePanelProps {
  ...
  mobile?: boolean;
  modelKind?: "avatar" | "audio" | "3d" | null;
}

const MODEL_KIND_KEYWORDS: Record<
  NonNullable<ExplorePanelProps["modelKind"]>,
  string[]
> = {
  avatar: ["avatar", "portrait", "character", "digital human", "face"],
  audio: ["audio", "music", "speech", "voice", "sound"],
  "3d": ["3d", "mesh", "model", "gaussian", "splat", "point cloud"],
};
...
export function ExplorePanel({
  ...
  const filteredModels = useMemo(() => {
    ...
    if (modelKind) {
      const keywords = MODEL_KIND_KEYWORDS[modelKind];
