...
export function FeaturedModelsPanel({
  ...
  const scopedFamilies = FEATURED_MODEL_FAMILIES.filter(
    (f) =>
      getModelWorkspace({
        model_id: f.primaryVariant,
        type: f.tags.join(" "),
      }) === workspace,
  );
  const posters = scopedFamilies.filter((f) => f.ratio === "poster");
  const squares = scopedFamilies.filter((f) => f.ratio === "square");
  const copy: Record<
    PlaygroundWorkspace,
  ...
  > = {
    image: {
      title: "精选图像模型",
      description: "精选图片模型，专注写实、人像、插画与高质量图像生成。",
    },
    video: {
      title: "精选视频模型",
    ...
    audio: {
      title: "精选音频模型",
      description: "精选音频模型，专注文字转语音、音色克隆、音乐和声音生成。",
    },
    "3d": {
      title: "精选 3D 模型",
      description: "精选 3D 模型，专注 mesh、Gaussian splat、物体和空间生成。",
    },
  };
  const { title, description } = copy[workspace];
  ...
  return (
    <div className="flex-1 overflow-y-auto overflow-x-hidden">
      <div className="p-3 space-y-3">
        ...
        {scopedFamilies.length === 0 && (
          <div className="rounded-xl border border-white/[0.06] bg-white/[0.03] px-4 py-8 text-center text-sm text-muted-foreground">
            当前分类暂无精选卡片，请在左侧模型选择器中选择对应模型。
          </div>
        )}
