...
const LEGACY_MODELS_CACHE_KEY = "wavespeed_models_cache_v1";
const MODELS_CACHE_PREFIX = "wavespeed_models_cache_v2";

let modelsFetchPromise: Promise<void> | null = null;
let modelsFetchBaseUrl = "";

function modelsCacheKeyForBaseUrl(baseUrl = apiClient.getBaseUrl()) {
  return `${MODELS_CACHE_PREFIX}:${encodeURIComponent(baseUrl)}`;
}
...
function loadCachedModels(): Model[] {
  try {
    const raw =
      localStorage.getItem(modelsCacheKeyForBaseUrl()) ||
      (apiClient.getBaseUrl() === "https://api.wavespeed.ai"
        ? localStorage.getItem(LEGACY_MODELS_CACHE_KEY)
        : null);
...
function saveCachedModels(models: Model[]) {
  try {
    localStorage.setItem(modelsCacheKeyForBaseUrl(), JSON.stringify(models));
  } catch {}
}
...
interface ModelsState {
  ...
  loadCachedModelsForCurrentService: () => void;
...
export const useModelsStore = create<ModelsState>((set, get) => ({
  ...
  hasFetched: false,
  lastBaseUrl: apiClient.getBaseUrl(),
  typeFiltersOpen: true,

  fetchModels: async (force = false) => {
    const baseUrl = apiClient.getBaseUrl();
    if (modelsFetchPromise && modelsFetchBaseUrl === baseUrl) {
      return modelsFetchPromise;
    }
    if (get().hasFetched && get().lastBaseUrl === baseUrl && !force) return;
    set({ isLoading: true, error: null });
    modelsFetchBaseUrl = baseUrl;
  ...
  loadCachedModelsForCurrentService: () => {
    set({
