...
const CANVAS_DOT_GAP = 20;
const CANVAS_SURFACE_COLOR = "#101010";
const CANVAS_DOT_PATTERN =
  "radial-gradient(circle at 0.75px 0.75px, rgba(255, 255, 255, 0.22) 0, rgba(255, 255, 255, 0.22) 0.42px, transparent 0.78px)";
const CANVAS_SURFACE_DEPTH =
  "radial-gradient(120% 85% at 50% 0%, rgba(255, 255, 255, 0.035) 0%, rgba(255, 255, 255, 0.012) 36%, rgba(255, 255, 255, 0) 70%)";
const DEFAULT_WORKFLOW_NODE_WIDTH = 380;
const DEFAULT_WORKFLOW_NODE_HEIGHT = 220;
const VIEWPORT_EMPTY_PADDING = 24;

function normalizeCanvasDotOffset(value: number, stride: number) {
  if (!Number.isFinite(value) || !Number.isFinite(stride) || stride <= 0) {
    return 0;
  }
  const next = value % stride;
  return next < 0 ? next + stride : next;
}

function getWorkflowNodeSize(node: Node) {
  const params = node.data?.params ?? {};
  const style = node.style ?? {};
  const width =
    Number(style.width) ||
    Number(node.width) ||
    Number(params.__nodeWidth) ||
    (node.type === "group-io" ? 220 : DEFAULT_WORKFLOW_NODE_WIDTH);
  const height =
    Number(style.height) ||
    Number(node.height) ||
    Number(params.__nodeHeight) ||
    (node.type === "group-io" ? 160 : DEFAULT_WORKFLOW_NODE_HEIGHT);
...
function getWorkflowNodeAbsolutePosition(
  node: Node,
...
function isAnyWorkflowNodeVisible(
  viewport: { x: number; y: number; zoom: number },
  flowWrapper: HTMLElement | null,
  nodes: Node[],
) {
  const visibleNodes = nodes.filter((node) => !node.hidden);
  if (visibleNodes.length === 0) return true;
  ...
  const padding = VIEWPORT_EMPTY_PADDING / zoom;
  const nodeById = new Map(nodes.map((node) => [node.id, node]));

  return visibleNodes.some((node) => {
    const { width: nodeWidth, height: nodeHeight } = getWorkflowNodeSize(node);
    const nodePosition = getWorkflowNodeAbsolutePosition(node, nodeById);
...
/** Zoom in/out and canvas controls, positioned like the LibTV bottom-left bar. */
function CanvasZoomControls({ zoom }: CanvasZoomControlsProps) {
  ...
  const buttonClass =
    "flex h-8 w-8 items-center justify-center rounded-md text-white/58 transition-colors hover:bg-white/8 hover:text-white";
  const activeButtonClass = "bg-white/8 text-primary";
  const tooltipSide = "top" as const;
  const zoomPercent = `${Math.round((Number.isFinite(zoom) ? zoom : 1) * 100)}%`;

  return (
    <div
      className="absolute bottom-4 left-4 z-20 flex items-center gap-1 rounded-full border border-white/[0.08] bg-[#202020]/72 px-2 py-1.5 text-white/70 shadow-[0_14px_40px_rgba(0,0,0,0.28)] backdrop-blur-md"
    ...
    >
      ...
      <Tooltip delayDuration={0}>
        <TooltipTrigger asChild>
          <button
            ...
            className={cn(
              buttonClass,
              interactionMode === "hand" && activeButtonClass,
            )}
          >
      ...
      <span className="min-w-[46px] px-1 text-center text-xs font-medium tabular-nums text-white/70">
        {zoomPercent}
      </span>
      {/* Zoom in */}
      <Tooltip delayDuration={0}>
...
export function WorkflowCanvas({ nodeDefs = [] }: WorkflowCanvasProps) {
  ...
  const canvasBackgroundStyle = useMemo(() => {
    const zoom =
      Number.isFinite(canvasViewport.zoom) && canvasViewport.zoom > 0
        ? canvasViewport.zoom
        : 1;
    const spacing = Math.max(
      14,
      Math.round(CANVAS_DOT_GAP * zoom * 1000) / 1000,
    );
    const offsetX =
      Math.round(normalizeCanvasDotOffset(canvasViewport.x, spacing) * 1000) /
      1000;
    const offsetY =
      Math.round(normalizeCanvasDotOffset(canvasViewport.y, spacing) * 1000) /
      1000;

    return {
      backgroundColor: CANVAS_SURFACE_COLOR,
      backgroundImage: `${CANVAS_DOT_PATTERN}, ${CANVAS_SURFACE_DEPTH}`,
      backgroundPosition: `${offsetX}px ${offsetY}px, center top`,
      backgroundRepeat: "repeat, no-repeat",
      backgroundSize: `${spacing}px ${spacing}px, 100% 100%`,
    };
  }, [canvasViewport.x, canvasViewport.y, canvasViewport.zoom]);
  ...
  const updateEmptyViewportHint = useCallback(
    (viewport?: { x: number; y: number; zoom: number }) => {
      const nextViewport = viewport ?? reactFlowInstance.current?.getViewport();
  ...
  const returnToContentNodes = useCallback(
    (event?: React.MouseEvent<HTMLButtonElement>) => {
      ...
      const targetNodes = displayNodesRef.current.filter(
        (node) => !node.hidden,
      );
      ...
      window.setTimeout(() => updateEmptyViewportHint(), 460);
    },
  ...
  useEffect(() => {
    ...
    updateEmptyViewportHint();
  }, [displayNodes, updateEmptyViewportHint]);
      ...
      <div ref={reactFlowWrapper} className="flex-1 h-full relative">
        <ReactFlow
          ...
          onInit={(instance) => {
            ...
            window.setTimeout(() => updateEmptyViewportHint(), 80);
          }}
          onMove={handleCanvasMove}
          onMoveEnd={(_event, viewport) => {
            handleCanvasMove(_event, viewport);
            updateEmptyViewportHint(viewport);
            const wfId = useWorkflowStore.getState().workflowId;
            if (wfId && viewport) {
              try {
        ...
        {showEmptyViewportHint && (
          <div className="pointer-events-none absolute inset-x-0 bottom-10 z-30 flex justify-center">
            <div
              className="pointer-events-auto flex items-center gap-4 rounded-full border border-white/[0.1] bg-[#202020]/90 px-4 py-2 text-white/[0.9] shadow-[0_18px_50px_rgba(0,0,0,0.34)] backdrop-blur-md"
            ...
            >
              ...
              <button
                type="button"
                className="inline-flex h-8 items-center justify-center whitespace-nowrap rounded-full bg-white px-4 text-xs font-medium text-black shadow transition-colors hover:bg-white/90"
        ...
        <SubgraphBreadcrumb />
        <CanvasZoomControls zoom={canvasViewport.zoom} />
