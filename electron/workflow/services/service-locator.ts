...
// We reuse Desktop's apiClient pattern but in main process we need to read settings directly.
let _apiKey: string = "";
let _apiBaseUrl: string = "";

function loadDesktopApiSettings(): { apiKey: string; apiBaseUrl: string } {
  if (_apiKey && _apiBaseUrl) {
    return { apiKey: _apiKey, apiBaseUrl: _apiBaseUrl };
  }
  try {
...
export function getWaveSpeedClient(): WaveSpeedMainClient {
  ...
  const { apiKey, apiBaseUrl } = loadDesktopApiSettings();
  ...
  _wsClient = {
    async run(
      ...
      while (true) {
        ...
        const pollRes = await withAbort(
          fetch(`${apiBaseUrl}/api/v3/predictions/${requestId}/result`, {
            headers: {
              Authorization: `Bearer ${apiKey}`,
              "X-Client-Name": "wavespeed-desktop-workflow",
            },
    ...
    async uploadFile(file: File, filename: string) {
      ...
      const res = await fetch(`${apiBaseUrl}/api/v3/media/upload/binary`, {
        method: "POST",
        headers: {
          Authorization: `Bearer ${apiKey}`,
          "X-Client-Name": "wavespeed-desktop-workflow",
